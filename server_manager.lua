...
serverManager.config = {
    keyGenChannel = 102,
    gatewayChannel = 101,  -- For registering with gateway
...
serverManager.machineCounter = 0
...
function serverManager.save()
    ...
    if file then
        file.write(textutils.serialize({
            ...
            machineCounter = serverManager.machineCounter
        }))
        file.close()
        return true
    end
    ...
    if fs.exists(serverManager.config.dataFile) then
        ...
        if file then
            ...
            if data then
                ...
                serverManager.machineCounter = data.machineCounter or 0
...
function serverManager.generateMachineId(machineType)
    serverManager.machineCounter = serverManager.machineCounter + 1
    local prefix = "UNKNOWN"
    
    if machineType == "deposit" then
        prefix = "DEPOSIT"
    elseif machineType == "payment" then
        prefix = "PAYMENT"
    elseif machineType == "terminal" then
        prefix = "TERMINAL"
    end
    
    return prefix .. "_" .. string.format("%03d", serverManager.machineCounter)
end
